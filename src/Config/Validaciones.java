...
public class Validaciones {
    ...
    public Map validarCamposAcceso(Map respuesta) {

        Acceso Ac = (Acceso) respuesta.get("Acceso");

        if (Ac.getUsuario().equalsIgnoreCase("")) {
            respuesta.put("Mensaje", "El campo Usuario se encuentra vacio");
            respuesta.put("campo", "TxtUsuario.requestFocusInWindow()");
            return respuesta;
        }
        if (Ac.getContrasena().equalsIgnoreCase("")) {
            respuesta.put("Mensaje", "El campo Contraseña se encuentra vacio");
...
//        System.out.println("valor vacio "+val);
    ...
    public Map validarCamposRoles(Map respuesta) {
        ...
        if (("" + Rl.getIdRol()).equals("")) {
            respuesta.put("Mensaje", "Error en el ID del Roll");
            return respuesta;
        }
        ...
        if (Rl.getPermisosMod() <= 0) {
            respuesta.put("Mensaje", "El Roll no tiene Permisos");
            respuesta.put("campo", "TxtSiglas.requestFocusInWindow()");
            return respuesta;
        }
        respuesta.remove("Mensaje");
